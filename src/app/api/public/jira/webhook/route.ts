...
async function readLimitedBody(request: Request): Promise<Uint8Array> {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) {
    throw new RangeError("Jira webhook payload is too large");
  }
  if (!request.body) throw new Error("Jira webhook payload is required");
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const result = await getServerServices().jiraWebhookService.handleWebhook({
      body,
      signature: request.headers.get("x-hub-signature"),
      deliveryId: request.headers.get("x-atlassian-webhook-identifier"),
      retryCount: request.headers.get("x-atlassian-webhook-retry"),
    });
