  ...
  withIntegrationConfigurationEvents(
    {
      Query: {
        ...
        jiraTicketSummary: (
          ...
          requireControlPlane(context);
          return jiraService.summary(issueKey);
        },
        jiraTicket: (
          _root: unknown,
          {
            issueKey,
            commentsFirst,
          }: { issueKey: string; commentsFirst?: number | null },
          context: GraphQLContext,
        ) => {
          requireControlPlane(context);
          return commentsFirst == null
            ? jiraService.ticket(issueKey)
            : jiraService.ticket(issueKey, false, undefined, commentsFirst);
        },
        jiraTicketComments: (
          _root: unknown,
          {
          ...
          requireControlPlane(context);
          return jiraService.ticketComments(
            issueKey,
            limit ?? 50,
            offset ?? 0,
            snapshotTotal,
        ...
        jiraTicketChanges: (
          ...
          requireControlPlane(context);
          return jiraService.ticketChanges(
            issueKey,
            limit,
            offset,
            snapshotTotal,
        ...
        jiraTicketWorklogs: (
          ...
          requireControlPlane(context);
          return jiraService.ticketWorklogs(
            issueKey,
            limit,
            offset,
            snapshotTotal,
      ...
      Mutation: {
        ...
        assignJiraTicket: (
          _root: unknown,
          {
            issueKey,
            accountId,
          }: { issueKey: string; accountId?: string | null },
          context: GraphQLContext,
        ) => {
          requireControlPlane(context);
