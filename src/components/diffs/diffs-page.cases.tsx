...
export type DiffsPageTestSection = "navigation" | "coverage" | "state";

export function registerDiffsPageTests(section: DiffsPageTestSection): void {
  describe("DiffsPage", () => {
    if (section === "navigation") {
      ...
      test("wraps long lines until the query string opts out", async () => {
        ...
        expect(
          opted.container.querySelector(".whitespace-pre-wrap"),
        ).toBeNull();
      });

      test("honors the initial selection from the query string", async () => {
        render(
          <DiffsPage initial={{ scope: "STAGED", path: "src/staged.ts" }} />,
        );
        const list = await fileList();
        expect(list.getByText("staged.ts")).toBeTruthy();
        expect(list.queryByText("app.ts")).toBeNull();
    ...
    if (section === "coverage") {
      ...
      test("joins the report's file coverage into the sidebar", async () => {
        ...
        expect(
          (await fileList()).getByText("zebra.ts").closest("button")!
            .textContent,
        ).not.toContain("%");
      });
      ...
      test("trades the change counts for coverage rings under the coverage sort", async () => {
        ...
        expect(
          (await row()).querySelector("[data-coverage-indicator]"),
        ).toBeNull();
        ...
        fireEvent.click(
          await screen.findByRole("option", { name: "Coverage" }),
        );
      ...
      test("selecting a report from the picker paints the gutter", async () => {
        ...
        await screen.findByText("new line");
        expect(
          container.querySelector('[data-coverage="uncovered"]'),
        ).toBeTruthy();
      });
    ...
    if (section === "state") {
      test("flags changed working-tree contents even when HEAD has not moved", async () => {
        ...
        render(
          <DiffsPage
            initial={{ scope: "STAGED", coverageReportId: "report-1" }}
          />,
        );
        expect(
          await screen.findByText("Measured at a different revision."),
      ...
      test("discards late detail responses after switching worktrees", async () => {
        const firstWorktree =
          overview.worktreeOverview.agents[0]!.codebases[0]!;
        ...
        request.mockImplementation((query, variables) => {
          ...
          if (operation.includes("mutation InspectWorktreeDiff")) {
            return Promise.resolve({
              inspectWorktreeDiff: commitDiff,
            }) as never;
          }
          return Promise.reject(
            new Error(`unexpected operation: ${operation}`),
          );
        });
