                    ...
                    <CardContent className="flex flex-wrap items-center justify-between gap-4">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h2 className="font-medium">{names.repository}</h2>
                        ...
                        <p className="mt-1 truncate font-mono text-xs text-muted-foreground">
                          {names.worktree} · {build.destination.name}
                        </p>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {date(build.startedAt ?? build.createdAt)}
                          {build.durationMs !== null
                            ? ` · ${t("durationSeconds", { count: Math.round(build.durationMs / 1000) })}`
                            : ""}
                          {build.artifacts.length
                            ? ` · ${t("artifactCount", { count: build.artifacts.length })}`
                            : ""}
                        </p>
                      </div>
                      <Button asChild size="sm" variant="outline">
                        <Link href={`/builds/${build.id}`}>
                          {t("viewBuild")} <ArrowRight />
                        </Link>
                      </Button>
                    </CardContent>
                  </Card>
...
function BuildScriptDialog({
  ...
  const [pre, setPre] = useState(script?.preBuildScript ?? "");
  const [post, setPost] = useState(script?.postBuildScript ?? "");
