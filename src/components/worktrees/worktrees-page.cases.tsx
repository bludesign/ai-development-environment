  ...
  describe("WorktreesPage", () => {
                  ...
                  worktrees: [
                    {
                      ...
                      latestBuild: {
                        ...
                        artifacts: [
                          { id: "app-artifact", kind: "RUNNABLE_APP" },
                        ],
                        createdAt: new Date(0).toISOString(),
                      },
                      tags: [
    ...
    if (section === "cards") {
      ...
      test("searches worktrees and provides shadcn agent and repository filters", async () => {
        ...
        expect(repositoryFilter.getAttribute("data-slot")).toBe(
          "select-trigger",
        );
    ...
    if (section === "details") {
      ...
      test("edits the base branch with an inline select", async () => {
        ...
        const editBase = screen.getByRole("button", {
      ...
      test("refreshes expanded details when live worktree activity arrives", async () => {
        ...
        expect(screen.getByText("Dirty")).toBeDefined();
        expect(
          screen.getByRole("button", { name: "Commits: 2" }),
        ).toBeDefined();
              ...
              if (operation.query.includes("WorktreeInspectionChanged")) {
                ...
                if (liveAttempts === 1) {
                  liveSubscription.fail = () =>
                    sink.error(new Error("temporary"));
                }
              }
              return vi.fn();
            },
    ...
    if (section === "filters") {
      ...
      test("falls back to a dash only when the pull request row is empty", async () => {
        ...
        await screen.findByText("feature/AIDE-24");
        expect(
          screen.getByRole("button", { name: "Commits: 1" }),
        ).toBeDefined();
        expect(screen.queryByText("—")).toBeNull();
      ...
      test("groups by repository and remembers the choice", async () => {
        ...
        fireEvent.click(
          screen.getByRole("combobox", { name: "Group worktrees" }),
        );
        fireEvent.click(
