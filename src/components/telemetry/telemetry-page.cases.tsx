...
export type TelemetryPageTestSection = "timeline" | "controls";

export function registerTelemetryPageTests(
  section: TelemetryPageTestSection,
): void {
  describe("TelemetryPage", () => {
    if (section === "timeline") {
      test("renders the console timeline, facets, counts, and expanded dictionaries", async () => {
        ...
        expect(
          screen.getByText("Message", { selector: "p" }).parentElement
            ?.className,
        ).toContain("px-3");
        expect(
    ...
    if (section === "controls") {
      ...
      test("shows and copies page-specific REST API documentation", async () => {
        ...
        fireEvent.click(
          screen.getByRole("button", { name: "Copy Markdown docs" }),
        );
